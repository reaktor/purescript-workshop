...
assertRecordArrayEqual :: forall r rs
                  . RowToList r rs
                  => EqualFields rs r
                  => SimpleJSON.WriteForeign (Record r)
                  => Array (Record r) -> Array (Record r) -> Test
...
assertRecordArrayEqual expected actual =
  let
    ...
    formatAndThrow e = throwError (Error.error $ "Record arrays contain unequal value:\n" <> Error.message e)
  in
   catchError (sequence comparisons) formatAndThrow *> pure unit

assertRecordEqual :: forall r rs
               . RowToList r rs
               => SimpleJSON.WriteForeign (Record r)
               => EqualFields rs r
               => Record r -> Record r -> Test
